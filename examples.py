...
def tabular_bias():
    help.header("Tabular Data", is_sidebar=False)
    ...
    with col1:
        help.sub_text("""<b>Keywords</b>""", alignment="left")
        help.sub_text(
            """
                      Normalization
                      <br>Sampling 
    ...
    with col2:
        help.sub_text("""<b>Packages used</b>""", alignment="left")
        help.sub_text(
            """
    ...
    with col3:
        help.sub_text("""<b>Example dataset</b>""", alignment="left")
        help.sub_text(
            """
    ...
    with col4:
        help.sub_text("""<b>Similar packages</b>""", alignment="left")
        help.sub_text(
            """
    ...
    expander = st.expander("Data Understanding", expanded=False)
    ...
    with expander:
        ...
        if button_du:
            ...
            with st.echo():
                ...
                st.write(
                    "Number of males: " + str(titanic["sex"].value_counts()["male"])
                )
                st.write(
                    "Number of females: " + str(titanic["sex"].value_counts()["female"])
                )
    ...
    expander = st.expander("Data Preparation", expanded=False)
    ...
    with expander:
        ...
        if button_dp:
            ...
            with st.echo():
                ...
                X_train_rnd = X[:200]
                y_train_rnd = y[:200]
                X_train_rnd = pd.concat([X_train_rnd, X.iloc[-25:-22]])
                y_train_rnd = pd.concat([y_train_rnd, y.iloc[-25:-22]])
                ...
                (
                    ...
                    test_size=0.33,
                    random_state=41,
                    stratify=X[0:-20]["Sex_female"],
                )

                # Check proportions after random sampling
                st.write(
                    "Proportion of females in trainset after stratisfied sampling: "
                    + str()
                )
                st.write(
                    len(X_train_strat[X_train_strat["Sex_female"] == 1])
                    / len(X_train_strat)
                )

    expander = st.expander("Model Development", expanded=False)
        ...
        if button_md:
            ...
            with st.echo():
                ...
                def accuracy_score(y_pred, y_test):
                    ...
                    b = list(y_test)
                    accuracy = len(
                        [a[i] for i in range(0, len(a)) if a[i] == b[i]]
                    ) / len(a)
                    return accuracy
                ...
                st.write(
                    "Accuracy of the model on men: ",
                    str(accuracy_score(y_test_rnd_male, y_pred)),
                )

                # Show accuracy on women
                y_pred = clf.predict(X_test_female)
                st.write(
                    "Accuracy of the model on women <b>before sampling</b>: "
                    + str(accuracy_score(y_test_female, y_pred))
                    + "\n",
                ...
                y_pred = clf.predict(X_test_female)
                st.write(
                    "Accuracy of the model on the women <b>after sampling</b>: "
                    + str(accuracy_score(y_test_female, y_pred))
                    + "\n",
                    unsafe_allow_html=True,
                )
...
def text_bias():
    help.header("Text Data", is_sidebar=False)
    ...
    expander = st.expander("Data Understading", expanded=False)
    ...
    expander = st.expander("Responsibly Example", expanded=False)
    ...
    with expander:
        ...
        if button_res:
            with st.echo():
                ...
                she = w2v_small.most_similar(
                    positive=["doctor", "she"], negative=["he"]
                )

                # get the most similar terms from model,
                # exclude words related to she/her
                he = w2v_small.most_similar(positive=["doctor", "he"], negative=["she"])
...
def tabular_xai():
    ...
    expander = st.expander("Model Development and Evaluation", expanded=False)
       ...
       """
        ...
        if button_me:
            with st.echo():
                ...
                st.pyplot()
                st.write(
                    "<b>SHAP summary dot plot</b>:  Depicts how different feature values ",
                    "contribute globally. ",
                ...
                st.pyplot()
                st.write(
                    "<b>SHAP Bar plot</b>: Global explanations depicting feature importance.",
                    unsafe_allow_html=True,
                )
                ...
                st.pyplot()
                st.write(
                    "<b>SHAP waterfall plot</b>: The waterfall plot also allows us to see  ",
                ...
                # st.pyplot()
                st.write(
                    "<b>SHAP dependence plot</b>: analyzes the features two by two by ",
                ...
                st.write(
                    "<b>SHAP force plot</b>: The force plot is good to see where the ",
...
def image_xai():
    help.header("Image Data", is_sidebar=False)
    ...
    expander = st.expander("NBDT Example", expanded=False)
...
With this approach, the accuracy of a neural net is preserved.">
    ...
    expander = st.expander("Lime Example", expanded=False)
       ...
       """
        ...
        if button_xai_me:
            with st.echo():
                ...
                Xi = skimage.io.imread(
                    "https://arteagac.github.io/blog/lime_image/img/cat-and-dog.jpg"
                )
                ...
                skimage.io.imshow(
                    skimage.segmentation.mark_boundaries(Xi / 2 + 0.5, superpixels)
                )

                # Generate perturbations
                num_perturb = 150
                perturbations = np.random.binomial(
                    1, 0.5, size=(num_perturb, num_superpixels)
                )
                ...
                skimage.io.imshow(
                    perturb_image(Xi / 2 + 0.5, perturbations[0], superpixels)
                )
...
def text_xai():
    ...
    expander = st.expander("LIME Explanation Example", expanded=False)
...
">LIME</dfn></span>. 
        ...
        if button_xai_txt:
            with st.echo():
                ...
                data_urls = [
                    "https://raw.githubusercontent.com/KaliaBarkai/KaggleDisasterTweets/master/Data/%s.csv"
                    % ds
                    for ds in ["train", "test", "sample_submission"]
                ]
            ...
            logreg = LogisticRegression(
                verbose=1, random_state=0, penalty="l2", solver="newton-cg"
            )
            ...
            st.write(
                ...
                c.predict_proba([ls_X_test[idx]]).round(3)[0, 1],
            )
...
def dvc_data():
    ...
    help.header("Data Version Control", is_sidebar=False)
    ...
    expander = st.expander("DVC Example", expanded=False)
...
def run_subprocess(command, show_output: bool = True):
    ...
    process = subprocess.run(
        array_command, text=show_output, capture_output=show_output
    )
